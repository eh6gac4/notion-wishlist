...
- 表示中の合計金額表示
- AI 分析（Gemini による「買うべきか」判定 → `分析結果` プロパティへ保存）

## AI 分析機能

詳細ダイアログの「分析する」ボタンを押すと、Gemini API がアイテム情報（品名・価格・優先度・登録経過日数・メモ等）を踏まえて「買う／見送る／保留」の結論と理由を返し、対象 Notion ページの本文末尾に「🤖 AI 分析（時刻）」見出し + 箇条書きとして追記します。再分析しても過去のブロックは削除されず、履歴として末尾に積み上がります（不要になれば Notion 側で手動削除可能）。
