...
| `購入予定日` | Date | 購入予定日 |
| `メモ` | Rich text | 備考 |
| `分析結果` | Rich text | AI 分析機能で書き込まれる判定文（空でも可） |
...
詳細ダイアログの「分析する」ボタンを押すと、Claude API がアイテム情報（品名・価格・優先度・登録経過日数・メモ等）を踏まえて「買う／見送る／保留」の結論と理由を返し、Notion DB の `分析結果` プロパティに保存します。
